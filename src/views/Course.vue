          ...
          <a-table size="small" :loading="isLoading" class="mb-12" :pagination="false" :columns="columns" :data="data"
            ...
            <template #product_img="{ record }">
              <a-upload :headers="{ token }" :disabled="!record.editable" :action="uploadApi"
                :fileList="[record.productImgFile]" :show-file-list="false"
                @success="(file) => { record.product_img = file.response[0].url }">
                <template #upload-button>
                  <div class="arco-upload-list-picture custom-upload-avatar">
                    <a-image :src="record.product_img" width="100" />
                    <div v-if="record.editable" class="arco-upload-list-picture-mask">
                      <IconEdit />
                    </div>
                  </div>
                </template>
            ...
            <template #optional="{ record }">
              <a-space direction="horizontal" size="small" v-if="record.id">
                <a-button size="small" type="secondary" status="normal"
                  @click="() => { goToVideo(record.id, record.product_name) }">视频</a-button>
            ...
            <template #product_introduce="{ record }">
              <a-textarea placeholder="课程简介" :disabled="!record.editable" v-model="record.product_introduce" auto-size />
            </template>
          </a-table>
...
import { getCourseProductInfo, updatesProductInfo, addProductInfo } from "@/api/product";
  ...
  setup() {
    ...
    const columns = [
      ...
      },
      {
        ...
        // render: (item: any) => {
        // return `￥${item.record?.product_price / 100 || 1}`;
        // },
      },
      {
        title: "状态",
    ...
    function getDataByPage(page: number) {
      ...
      curPage.value = page;
      getCourseProductInfo(page, currentStatus.value).then((res: any) => {
    ...
    function validateInfo(record: any) {
      const { product_name, product_introduce, product_sort, product_price, product_img } = record;
    ...
    function goToVideo(id: string, name: string) {
      router.push({
        path: '/product/video',
        query: {
          id,
